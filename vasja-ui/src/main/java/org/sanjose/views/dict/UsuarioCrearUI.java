package org.sanjose.views.dict;

import com.vaadin.annotations.AutoGenerated;
import com.vaadin.annotations.DesignRoot;
import com.vaadin.ui.declarative.Design;
import com.vaadin.ui.HorizontalLayout;
import com.vaadin.ui.ComboBox;
import com.vaadin.ui.TextField;
import com.vaadin.ui.Button;
import com.vaadin.ui.CheckBox;
import com.vaadin.ui.PasswordField;

/**
 * !! DO NOT EDIT THIS FILE !!
 * <p>
 * This class is generated by Vaadin Designer and will be overwritten.
 * <p>
 * Please make a subclass with logic and additional interfaces as needed,
 * e.g class LoginView extends LoginDesign implements View { }
 */
@DesignRoot
@AutoGenerated
@SuppressWarnings("serial")
public class UsuarioCrearUI extends HorizontalLayout {
    protected TextField codUsuario;
    protected TextField txtUsuario;
    protected TextField txtNombre;
    protected ComboBox codRol;
    protected TextField txtAplicacion;
    protected TextField txtCorreo;
    protected CheckBox chkEnviarInvitacion;
    protected PasswordField txtPass1;
    protected PasswordField txtPass2;
    protected Button btnGuardar;
    protected Button btnEliminar;
    protected Button btnAnular;
    protected Button btnNuevo;

    public UsuarioCrearUI() {
        Design.read(this);
    }

}
